...
export async function addWallets(walletAddresses: Address[], data: WalletData) {
  ...
  await Promise.all(walletAddresses.map((walletAddress: Address) => walletStore.setJSON(walletAddress.toLowerCase(), data)))
}

export async function removeWallets(walletAddresses: Address[]) {
  const walletStore = getWalletStore()

  await Promise.all(walletAddresses.map((walletAddress: Address) => walletStore.delete(walletAddress.toLowerCase())))
}
