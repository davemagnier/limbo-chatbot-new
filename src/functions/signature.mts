...
if (!SbtContractAddress || !SbtContractName || !SbtContractVersion || !sbtAuthPrivateKey || !messageAuthPrivateKey || !encryptionKey || !rpcUrl) {
...
const app = new Hono<{ Variables: Variables }>().basePath('/api/v1/signature').use('*', sessionAuth)
...
app.get('/mint', async (c) => {
  ...
  return c.json({ signaure, walletAddress: session.walletAddress, contract: SbtContractAddress, chainId, tokenId, message: messageHash })
})
